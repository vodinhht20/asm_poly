      ...
      <div id="kt_datatable_wrapper" class="dataTables_wrapper dt-bootstrap4 no-footer"><div class="row"><div class="col-sm-12"><table class="table table-bordered table-checkable dataTable no-footer dtr-inline collapsed" id="kt_datatable" role="grid" aria-describedby="kt_datatable_info" style="width: 1234px;">
        <thead>
          <tr role="row">
            ...
            <th style="width:220px;" class="sorting" rowspan="1" colspan="1" style="text-align: center"  aria-label="Actions">Actions</th>
          </tr>
        </thead>
          ...
          @foreach ($product as $prod)
            ...
            <tr>
              ...
              <td style="display: grid-flex; text-align: center">
                ...
                <a href="{{route('edit.product', ['token' => $prod->token])}}" class="btn btn-primary " @popper(Chỉnh Sửa)><i class="fa	fa-edit"></i></a>  
                <a onclick="return confirm('Bạn Có Muốn Phê Duyệt Sản Phẩm Này Không??')" href="{{route('preview/accept', ['id' => $prod->id])}}" class="btn btn-primary" @popper(Phê Duyệt)><i class="fa	fa-check"></i></a>
                <div class="btn-group dropright">
                  <button type="button" class="btn btn-danger dropdown-toggle " data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                  </button>
                  <div class="dropdown-menu" >
                    <button id="reject_modal" class="reject_modal btn btn-danger m-2" data-id="{{$prod->id}}" data-url="{{route('preview/reject')}}" @popper(Từ Chối)><i class=" fa fa-ban"></i></button>
                    <a  class=" btn btn-danger" href="{{route('preview/del', ['id' => $prod->id])}}" onclick="return confirm('Bạn Có Muốn Xóa Sản Phẩm Này Không')"  @popper(Xóa)><i class="fa fa-trash-alt"></i></a>
                  </div>
                </div>  
