         ...
         <h3>Môn Học</h3>
          ...
          <div class="search">
            <div>
             ...
             <a href="{{route('bo-mon/add')}}" class="btn btn-outline-brand m-btn m-btn--custom m-btn--icon m-btn--pill m-btn--air" @popper(Thêm Mới)>
               <span>
        ...
        <div id="kt_datatable_wrapper" class="dataTables_wrapper dt-bootstrap4 no-footer"><div class="row"><div class="col-sm-12"><table class="table table-bordered table-checkable dataTable no-footer dtr-inline collapsed" id="kt_datatable" role="grid" aria-describedby="kt_datatable_info" style="width: 1234px;">
          ...
          <thead>
            <tr role="row">
              ...
              <th class="sorting" tabindex="0" aria-controls="kt_datatable" rowspan="1" colspan="1" >Tên Bộ Môn</th>
          ...
          <tbody id="list-content">
            @foreach ($main_major as $mj)
                ...
                <tr class="odd">
                  <td >{{$mj->id}}</td>
                  <td>{{$mj->name}}</td>
                  <td style="text-align: center" ><a href="{{route('bo-mon/edit',['id' => $mj->id])}}" class="btn btn-primary " @popper(Chỉnh Sửa)><i class="fa	fa-edit"></i></a> 
                                                  <a onclick="return confirm('Bạn Có Muốn Xóa Môn Học Này Không??')" href="{{route('bo-mon/del',['id' => $mj->id])}}" class="btn btn-danger" @popper(Xóa)><i class="fa fa-trash-alt"></i></a></td>
                    </tr>
            @endforeach
          </tbody>
        </table>
