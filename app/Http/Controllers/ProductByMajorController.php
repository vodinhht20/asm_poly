...
class ProductByMajorController extends Controller
{
    ...
    public function ListProductByMajor(Request $request, $slug) {
        ...
        if ($request->input("type")) {
            $products= $products->where('products.type_id', $request->input("type"));
        } 

        // lọc theo kỳ học
        if ($request->input("semester")) {
            $products= $products->where('products.semester', $request->input("semester"));
        }
        ...
        $major_name = Majors::find($idMajor)->name;
        return view("page.list_product.list_products_by_marjor",compact('products','semester','product_type','major_name'));
    }
